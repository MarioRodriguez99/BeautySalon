...
namespace BeautySalon.Frontend.Repositories
{
    public class Repository : IRepository
    {
        ...
        {
            var messegeJson = JsonSerializer.Serialize(model);
            var messegeContent = new StringContent(messegeJson, Encoding.UTF8, "application/json");
            var responseHttp = await _httpClient.PostAsync(url, messegeContent);
            if (responseHttp.IsSuccessStatusCode)
            {
